...
function pass_fail() {
    ...
    if [[ "$exit_code" -eq 0 ]]; then
        print "${green_text}PASS: $test_name${reset_text}"
    else 
        print "${red_text}FAIL: $test_name${reset_text}"
    fi
}
...
function deploy_test_env() {
    print "-> Creating the metro storage class"
    kubectl apply -f ./resources/sc.yaml
    if [ $? -ne 0 ]; then
        echo "FAIL: Failed to create storage class"
        return 1
    fi

    print "-> Creating the test environment"
    kubectl apply -f ./resources/deployment.yaml
    if [ $? -ne 0 ]; then
        echo "FAIL: Failed to create test environment"
        return 1
    fi
}
...
function test_pv_provisioning() {
    print "-> Waiting for PV to be provisioned and PVC to bind..."
    ...
    while true; do
        ...
        if [[ "$pvc_status" == "Bound" ]]; then
            echo "PASS: PV is provisioned for the PVC"
            break
        fi
...
function test_pod_running() {
    ...
    print "-> Waiting for pod $pod_name to be ready..."
...
function test_storage_multipath() {
    ...
    if [[ "${#mountpoints[@]}" -lt 4 ]]; then
        echo "FAIL: Not enough mount paths found for volume '$volume_name'"
    ...
    for device_path in "${mountpoints[@]}"; do
        ...
        if [[ $? -ne 0 ]]; then
            echo "FAIL: Could not find device path '$device_path' in multipath"
            return 1
        fi
    ...
    print "-> Waiting for paths to be 'active ready'"
    while true; do
        local is_ready=0
        ...
        if [[ "$is_ready" -eq 1 ]]; then
            echo "PASS: All paths are 'active ready'"
            break
        fi
...
function test_volume_availability() {
    ...
    print "-> Checking file system is writable"
    write_to_pod_fs "test-sanity.txt"
    if [[ $? -ne 0 ]]; then
        echo "FAIL: Failed to write to volume"
        return 1
    fi

    # determine which pair of paths belong to the same storage array.
    print "-> Getting LUN for device paths"
    ...
    print "-> Taking down one side of the metro replication session"
    ...
    for path in "${paths_to_take_down[@]}"; do
        ...
        if [[ "$response" != "ok" ]]; then
            echo "FAIL: Could not take down $path"
            return 1
        fi

        print "-> Waiting for paths to transition to 'failed' state"
        while true; do

            ssh_exec $host_ip 'multipathd show paths' | grep $path | grep 'failed faulty' > /dev/null
            if [[ $? -eq 0 ]]; then
                echo "PASS: $path was taken down"
                break
            fi
    ...
    print "-> Confirming the file system is still writable"
    for i in {1..10}; do
...
function test_suite_provisioning() {
    print "-> Starting Provisioning test"
...
function test_suite_volume_availability() {
    print "-> Starting Volume Availability test"
...
function clean_test_env() {
    ...
    print "-> Deleting artifacts in namespace $namespace"
    kubectl delete ns $namespace --wait=false
    
    print "-> Waiting for namespace '$namespace' to be deleted"
    while true; do
        kubectl get ns $namespace > /dev/null
        if [[ $? != 0 ]]; then
            echo "PASS: namespace '$namespace' successfully deleted"
            break
        fi
    ...
    print "-> Waiting for PV '$pv_name' to be deleted"
    while true; do
        kubectl get pv $pv_name > /dev/null
        if [[ $? != 0 ]]; then
            echo "PASS: PV '$pv_name' successfully deleted"
            break
        fi
