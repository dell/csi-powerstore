Feature: PowerStore CSI interface
  ...
  @current
  Scenario: Idempotent create nfs volume with duplicate name
    Given a PowerStore service on "controller"
    When I call Probe
    And I call CreateExistingNfsVolume "volume2" "16"
    Then a valid CreateVolumeResponse is returned
  ...
  @current
  Scenario: Idempotent create nfs volume with duplicate name and different sizes
    Given a PowerStore service on "controller"
    When I call Probe
    And I call CreateExistingNfsVolumeIncompatibleSize "volume3" "8"
    Then the error contains "already exists but is incompatible volume size"
  ...
  @current
  Scenario: Сreate nfs volume with duplicate name and error
    Given a PowerStore service on "controller"
    When I call Probe
    And I call CreateExistingNfsVolumeError "volume4" "8"
    Then the error contains "can't find volume"
 ...
 @current
  Scenario: Delete volume, but snaps exist
    Given a PowerStore service on "controller"
    When I call Probe
    And I call DeleteVolumeSnapError "volume1"
    Then the error contains "Snapshots based on this volume still exist"

  @current
  Scenario: Delete NFS volume, but snaps exist
    Given a PowerStore service on "controller"
    When I call Probe
    And I call DeleteNFSVolumeSnapError "volume1"
    Then the error contains "Snapshots based on this volume still exist"
  ...
  @current
  Scenario: Delete non-existing nfs volume
    Given a PowerStore service on "controller"
    When I call Probe
    And I call DeleteNonExistingNfsVolume "volume3"
    Then a valid DeleteVolumeResponse is returned

  @current
  Scenario: Delete volume but there is an error
    Given a PowerStore service on "controller"
    When I call Probe
    And I call DeleteVolumeError "volume3"
    Then a valid DeleteVolumeResponse is returned
  ...
  @current
  Scenario: Create nfs snapshot good scenario
    Given a PowerStore service on "controller"
    When I call Probe
    And I call CreateNfsSnapshot "snap1" "39bb1b5f"
    Then a valid CreateSnapshotResponse is returned
  ...
  @current
  Scenario: Idempotent create nfs snapshot with duplicate name
    Given a PowerStore service on "controller"
    When I call Probe
    And I call CreateExistingNfsSnapshot "snap1" "39bb1b5f"
    Then a valid CreateSnapshotResponse is returned
  ...
  Scenario: Сreate snapshot with duplicate name and error
    ...
    Then the error contains "rpc error: code = Internal desc = can't find volume snapshot 'snap2'"
  ...
  @current
  Scenario: Delete nfs snapshot good scenario
    Given a PowerStore service on "controller"
    When I call Probe
    And I call DeleteNfsSnapshot "39bb1b5f"
    Then a valid DeleteSnapshotResponse is returned
  ...
  @current
  Scenario: Delete non-existing nfs snapshot
    Given a PowerStore service on "controller"
    When I call Probe
    And I call DeleteNonExistingNfsSnapshot "snap2"
    Then a valid DeleteSnapshotResponse is returned

  @current
  Scenario: No snapshots to delete
    Given a PowerStore service on "controller"
    When I call Probe
    And I call DeleteSnapshotButNoneFound "snap2"
    Then a valid DeleteSnapshotResponse is returned
  ...
  @current
  Scenario: Create nfs from snapshot good scenario
    Given a PowerStore service on "controller"
    When I call Probe
    And I call CreateNfsFromSnapshot "16"
    Then a valid CreateVolumeResponse is returned
  ...
  @current
  Scenario: Create nfs from snapshot with incorrect size
    Given a PowerStore service on "controller"
    When I call Probe
    And I call CreateNfsFromSnapshotIncompatible "12"
    Then the error contains "rpc error: code = InvalidArgument desc = snapshot 39bb1b5f has incompatible size 16777216 bytes (additional 1610612736 bytes) with requested 12582912 bytes"
  ...
  @current
  Scenario: Create nfs from snapshot and snapshot not found
    Given a PowerStore service on "controller"
    When I call Probe
    And I call CreateNfsFromSnapshotError "16"
    Then the error contains "rpc error: code = NotFound desc = snapshot not found: 39bb1b5f"
  ...
  @current
  Scenario: Create nfs from snapshot failure scenario
    Given a PowerStore service on "controller"
    When I call Probe
    And I call failure CreateNfsFromSnapshot "16"
    Then the error contains "rpc error: code = Internal desc = can't create fs: 39bb1b5f"

  @current
  Scenario: Delete NFS volume good scenario
    Given a PowerStore service on "controller"
    When I call Probe
    And I call DeleteNFSVolume "volume1"
    Then a valid DeleteVolumeResponse is returned

  @current
  Scenario: Create NFS volume good scenario
    Given a PowerStore service on "controller"
    When I call Probe
    And I call CreateNFSVolume "volume1" "16"
    Then a valid CreateVolumeResponse is returned
  ...
  Scenario: Clone fs
    Given a PowerStore service on "controller"
    When I call Probe
    And I call CreateCloneFs "16"
    Then a valid CreateVolumeResponse is returned

  @current
  Scenario: Clone fs with incorrect size
    Given a PowerStore service on "controller"
    When I call Probe
    And I call CreateCloneFsIncompatible "12"
    Then the error contains "rpc error: code = InvalidArgument desc = fs 39bb1b5f has incompatible size 16777216 bytes (additional 1610612736 bytes) with requested 12582912 bytes"

  @current
  Scenario: Clone fs and fs not found
    Given a PowerStore service on "controller"
    When I call Probe
    And I call CreateCloneFsError "16"
    Then the error contains "rpc error: code = NotFound desc = fs not found: 39bb1b5f"

  @current
  Scenario: Create fs and clone fails
    Given a PowerStore service on "controller"
    When I call Probe
    And I call failure CreateCloneFs "16"
    Then the error contains "rpc error: code = Internal desc = can't create fs: 39bb1b5f"

  @current
  Scenario: Clone volume
    Given a PowerStore service on "controller"
    When I call Probe
    And I call CreateCloneVolume "16"
    Then a valid CreateVolumeResponse is returned

  @current
  Scenario: Clone volume with incorrect size
    Given a PowerStore service on "controller"
    When I call Probe
    And I call CreateCloneVolumeIncompatible "12"
    Then the error contains "rpc error: code = InvalidArgument desc = volume 39bb1b5f has incompatible size 16777216 bytes with requested 12582912 bytes"

  @current
  Scenario: Clone volume and volume not found
    Given a PowerStore service on "controller"
    When I call Probe
    And I call CreateCloneVolumeError "16"
    Then the error contains "rpc error: code = NotFound desc = volume not found: 39bb1b5f"

  @current
  Scenario: Create volume and clone fails
    Given a PowerStore service on "controller"
    When I call Probe
    And I call failure CreateCloneVolume "16"
    Then the error contains "rpc error: code = Internal desc = can't create volume: 39bb1b5f"

  @current
  Scenario: Expand Volume Size good
    Given a PowerStore service on "controller"
    When I call Probe
    And I call ExpandVolumeSize
    Then a valid ExpandVolumeResponse is returned

  @current
  Scenario: Expand NFS Volume Size good
    Given a PowerStore service on "controller"
    When I call Probe
    And I call ExpandNFSVolumeSize
    Then a valid ExpandVolumeResponse is returned

  @current
  Scenario: Expand Volume Size exceed limit
    Given a PowerStore service on "controller"
    When I call Probe
    And I call ExpandVolumeSizeExceedlimit
    Then a valid ExpandVolumeResponse is returned
    And the error contains "rpc error: code = OutOfRange desc = Volume exceeds allowed limit"

  @current
  Scenario: Expand Volume Size exceed limit
    Given a PowerStore service on "controller"
    When I call Probe
    And I call ExpandNFSVolumeSizeExceedlimit
    Then a valid ExpandVolumeResponse is returned
    And the error contains "rpc error: code = OutOfRange desc = Volume exceeds allowed limit"


  @current
  Scenario: Shrink volume size
    Given a PowerStore service on "controller"
    When I call Probe
    And I call ExpandVolumeSizeShrink
    Then a valid ExpandVolumeResponse is returned

  @current
  Scenario: Shrink NFS volume size
    Given a PowerStore service on "controller"
    When I call Probe
    And I call ExpandNFSVolumeSizeShrink
    Then a valid ExpandVolumeResponse is returned