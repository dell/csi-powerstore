...
func (s *Service) startAPIService(ctx context.Context) {
	...
	pollingFrequencyInSeconds = commonutils.SetPollingFrequency(ctx)
...
func (s *Service) apiRouter(_ context.Context) {
	log.Infof("starting http server on port %s", commonutils.APIPort)
	// create a new mux router
	router := mux.NewRouter()
	// route to connectivity status
	// connectivityStatus is the handlers
	router.HandleFunc(commonutils.ArrayStatus, connectivityStatus).Methods("GET")
	router.HandleFunc(commonutils.ArrayStatus+"/"+"{arrayId}", getArrayConnectivityStatus).Methods("GET")
	// start http server to serve requests
	server := &http.Server{
		Addr:         commonutils.APIPort,
		Handler:      router,
		ReadTimeout:  commonutils.Timeout,
		WriteTimeout: commonutils.Timeout,
	}
...
func MarshalSyncMapToJSON(m *sync.Map) ([]byte, error) {
	tmpMap := make(map[string]commonutils.ArrayConnectivityStatus)
	m.Range(func(k, value interface{}) bool {
		// this check is not necessary but just in case is someone in future play around this
		switch value.(type) {
		case commonutils.ArrayConnectivityStatus:
			tmpMap[k.(string)] = value.(commonutils.ArrayConnectivityStatus)
			return true
		default:
...
func (s *Service) startNodeToArrayConnectivityCheck(ctx context.Context) {
	...
	for _, array := range powerStoreArray {
		...
		go s.testConnectivityAndUpdateStatus(ctx, array, commonutils.Timeout)
	}
...
func (s *Service) testConnectivityAndUpdateStatus(ctx context.Context, array *array.PowerStoreArray, timeout time.Duration) {
	...
	for {
		...
		if existingStatus, ok := probeStatus.Load(array.GlobalID); !ok {
			...
			if status, ok = existingStatus.(commonutils.ArrayConnectivityStatus); !ok {
...
func (s *Service) populateTargetsInCache(array *array.PowerStoreArray) {
	...
	if s.useNVME[array.GlobalID] {
		...
		if s.useFC[array.GlobalID] {
			nvmefcInfo, err := commonutils.GetNVMEFCTargetInfoFromStorage(array.GetClient(), "")
			if err != nil {
			...
			// for NVMeTCP
			infoList, err := commonutils.GetNVMETCPTargetsInfoFromStorage(array.GetClient(), "")
			if err != nil {
		...
		infoList, err := commonutils.GetISCSITargetsInfoFromStorage(array.GetClient(), "")
		if err != nil {
			log.Errorf("couldn't get targets from array: %s", err.Error())
			return
		}
