...
var ReachableEndPoint = commonutils.ReachableEndPoint
...
func (n *NFSStager) Stage(ctx context.Context, req *csi.NodeStageVolumeRequest,
	...
	hostIP := req.PublishContext[commonutils.KeyHostIP]
	exportID := req.PublishContext[commonutils.KeyExportID]
	nfsExport := req.PublishContext[commonutils.KeyNfsExportPath]
	allowRoot := req.PublishContext[commonutils.KeyAllowRoot]
	nasName := req.PublishContext[commonutils.KeyNasName]

	natIP := ""
	if ip, ok := req.PublishContext[commonutils.KeyNatIP]; ok {
		natIP = ip
	}
	...
	logFields["NFSv4ACLs"] = req.PublishContext[commonutils.KeyNfsACL]
	...
	mode := os.ModePerm
	acls := req.PublishContext[commonutils.KeyNfsACL]
...
func readSCSIInfoFromPublishContext(publishContext map[string]string, useFC bool, useNVMe bool, isRemote bool) (scsiPublishContextData, error) {
	...
	deviceWwnKey := commonutils.PublishContextDeviceWWN
	lunAddressKey := commonutils.PublishContextLUNAddress
	if isRemote {
		deviceWwnKey = commonutils.PublishContextRemoteDeviceWWN
		lunAddressKey = commonutils.PublishContextRemoteLUNAddress
	}
...
func (s *SCSIStager) connectISCSIDevice(ctx context.Context,
	...
	connectorCtx = commonutils.SetLogFields(connectorCtx, logFields)
