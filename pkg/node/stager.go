...
var ReachableEndPoint = powerstorecommon.ReachableEndPoint
...
func (s *SCSIStager) Stage(ctx context.Context, req *csi.NodeStageVolumeRequest,
	...
	ctx = powerstorecommon.SetLogFields(ctx, logFields)
	...
	mntFlags := powerstorecommon.GetMountFlags(req.GetVolumeCapability())
...
func (n *NFSStager) Stage(ctx context.Context, req *csi.NodeStageVolumeRequest,
	...
	hostIP := req.PublishContext[powerstorecommon.KeyHostIP]
	exportID := req.PublishContext[powerstorecommon.KeyExportID]
	nfsExport := req.PublishContext[powerstorecommon.KeyNfsExportPath]
	allowRoot := req.PublishContext[powerstorecommon.KeyAllowRoot]
	nasName := req.PublishContext[powerstorecommon.KeyNasName]

	natIP := ""
	if ip, ok := req.PublishContext[powerstorecommon.KeyNatIP]; ok {
		natIP = ip
	}
	...
	logFields["NFSv4ACLs"] = req.PublishContext[powerstorecommon.KeyNfsACL]
	...
	mode := os.ModePerm
	acls := req.PublishContext[powerstorecommon.KeyNfsACL]
...
func readSCSIInfoFromPublishContext(publishContext map[string]string, useFC bool, useNVMe bool, isRemote bool) (scsiPublishContextData, error) {
	...
	deviceWwnKey := powerstorecommon.PublishContextDeviceWWN
	lunAddressKey := powerstorecommon.PublishContextLUNAddress
	if isRemote {
		deviceWwnKey = powerstorecommon.PublishContextRemoteDeviceWWN
		lunAddressKey = powerstorecommon.PublishContextRemoteLUNAddress
	}
...
func (s *SCSIStager) connectDevice(ctx context.Context, data scsiPublishContextData) (string, error) {
	logFields := powerstorecommon.GetLogFields(ctx)
	var err error
...
func (s *SCSIStager) connectISCSIDevice(ctx context.Context,
	...
	connectorCtx = powerstorecommon.SetLogFields(connectorCtx, logFields)
