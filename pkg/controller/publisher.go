...
func (s *SCSIPublisher) Publish(ctx context.Context, publishContext map[string]string, req *csi.ControllerPublishVolumeRequest,
	...
	if err != nil {
		if apiError, ok := err.(gopowerstore.APIError); ok && apiError.HostIsNotExist() {
			...
			ipList := powerstorecommon.GetIPListFromString(kubeNodeID)
...
func (s *SCSIPublisher) addLUNIDToPublishContext(
	...
	if !isRemote {
		publishContext[powerstorecommon.PublishContextDeviceWWN] = strings.TrimPrefix(volume.Wwn, powerstorecommon.WWNPrefix)
		publishContext[powerstorecommon.PublishContextLUNAddress] = strconv.FormatInt(mapping.LogicalUnitNumber, 10)
	} else {
		publishContext[powerstorecommon.PublishContextRemoteDeviceWWN] = strings.TrimPrefix(volume.Wwn, powerstorecommon.WWNPrefix)
		publishContext[powerstorecommon.PublishContextRemoteLUNAddress] = strconv.FormatInt(mapping.LogicalUnitNumber, 10)
	}
}
...
func (s *SCSIPublisher) addTargetsInfoToPublishContext(
	...
	iscsiPortalsKey := powerstorecommon.PublishContextISCSIPortalsPrefix
	iscsiTargetsKey := powerstorecommon.PublishContextISCSITargetsPrefix
	fcWwpnKey := powerstorecommon.PublishContextFCWWPNPrefix
	nvmeFcPortalsKey := powerstorecommon.PublishContextNVMEFCPortalsPrefix
	nvmeFcTargetsKey := powerstorecommon.PublishContextNVMEFCTargetsPrefix
	nvmeTCPPortalsKey := powerstorecommon.PublishContextNVMETCPPortalsPrefix
	nvmeTCPTargetsKey := powerstorecommon.PublishContextNVMETCPTargetsPrefix
	if isRemote {
		iscsiPortalsKey = powerstorecommon.PublishContextRemoteISCSIPortalsPrefix
		iscsiTargetsKey = powerstorecommon.PublishContextRemoteISCSITargetsPrefix
		fcWwpnKey = powerstorecommon.PublishContextRemoteFCWWPNPrefix
		nvmeFcPortalsKey = powerstorecommon.PublishContextRemoteNVMEFCPortalsPrefix
		nvmeFcTargetsKey = powerstorecommon.PublishContextRemoteNVMEFCTargetsPrefix
		nvmeTCPPortalsKey = powerstorecommon.PublishContextRemoteNVMETCPPortalsPrefix
		nvmeTCPTargetsKey = powerstorecommon.PublishContextRemoteNVMETCPTargetsPrefix
	}

	iscsiTargetsInfo, err := powerstorecommon.GetISCSITargetsInfoFromStorage(client, volumeApplianceID)
	if err != nil {
	...
	fcTargetsInfo, err := powerstorecommon.GetFCTargetsInfoFromStorage(client, volumeApplianceID)
	if err != nil {
	...
	nvmefcTargetInfo, err := powerstorecommon.GetNVMEFCTargetInfoFromStorage(client, volumeApplianceID)
	if err != nil {
	...
	nvmetcpTargetInfo, err := powerstorecommon.GetNVMETCPTargetsInfoFromStorage(client, volumeApplianceID)
	if err != nil {
...
func (n *NfsPublisher) Publish(ctx context.Context, publishContext map[string]string, req *csi.ControllerPublishVolumeRequest, client gopowerstore.Client,
	...
	if n.ExternalAccess != "" && !powerstorecommon.ExternalAccessAlreadyAdded(export, n.ExternalAccess) {
		externalAccess, err := powerstorecommon.GetIPListWithMaskFromString(n.ExternalAccess)
		if err != nil {
	...
	publishContext[powerstorecommon.KeyNfsExportPath] = fileInterface.IPAddress + ":/" + export.Name
	publishContext[powerstorecommon.KeyHostIP] = ipWithNat[0]
	if n.ExternalAccess != "" {
		parsedExternalAccess, _ := powerstorecommon.GetIPListWithMaskFromString(n.ExternalAccess)
		publishContext[powerstorecommon.KeyNatIP] = parsedExternalAccess
	}
	publishContext[powerstorecommon.KeyExportID] = export.ID
	publishContext[powerstorecommon.KeyAllowRoot] = req.VolumeContext[powerstorecommon.KeyAllowRoot]
	publishContext[powerstorecommon.KeyNfsACL] = req.VolumeContext[powerstorecommon.KeyNfsACL]
	return &csi.ControllerPublishVolumeResponse{PublishContext: publishContext}, nil
}
