...
func setVolumeCreateAttributes(reqParams map[string]string, createParams *gopowerstore.VolumeCreate) {
	if applianceID, ok := reqParams[identifiers.KeyApplianceID]; ok {
		createParams.ApplianceID = applianceID
	}
	if description, ok := reqParams[identifiers.KeyVolumeDescription]; ok {
	...
	if appType, ok := reqParams[identifiers.KeyAppType]; ok {
		createParams.AppType = gopowerstore.AppTypeEnum(appType)
		if appTypeOther, ok := reqParams[identifiers.KeyAppTypeOther]; ok {
...
func setFLRAttributes(reqParams map[string]string, createParams *gopowerstore.FsCreate) {
	flrMode, flrModeFound := reqParams[identifiers.KeyFlrCreateMode]
	flrDefaultRetention, flrDefaultRetentionFound := reqParams[identifiers.KeyFlrDefaultRetention]
	flrMinimumRetention, flrMinimumRetentionFound := reqParams[identifiers.KeyFlrMinRetention]
	flrMaximumRetention, flrMaximumRetentionFound := reqParams[identifiers.KeyFlrMaxRetention]
...
func setNFSCreateAttributes(reqParams map[string]string, createParams *gopowerstore.FsCreate) {
	...
	if configType, ok := reqParams[identifiers.KeyConfigType]; ok {
		createParams.ConfigType = configType
	}
	if accessPolicy, ok := reqParams[identifiers.KeyAccessPolicy]; ok {
		createParams.AccessPolicy = accessPolicy
	}
	if lockingPolicy, ok := reqParams[identifiers.KeyLockingPolicy]; ok {
		createParams.LockingPolicy = lockingPolicy
	}
	if folderRenamePolicy, ok := reqParams[identifiers.KeyFolderRenamePolicy]; ok {
		createParams.FolderRenamePolicy = folderRenamePolicy
	}
	if isAsyncMTimeEnabled, ok := reqParams[identifiers.KeyIsAsyncMtimeEnabled]; ok {
	...
	if hostIOSize, ok := reqParams[identifiers.KeyHostIoSize]; ok {
...
func (sc *SCSICreator) Create(ctx context.Context, req *csi.CreateVolumeRequest, sizeInBytes int64, client gopowerstore.Client) (gopowerstore.CreateResponse, error) {
	...
	} else if vgID, ok := req.Parameters[identifiers.KeyVolumeGroupID]; ok {
