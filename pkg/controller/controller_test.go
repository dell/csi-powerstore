...
func setVariables() {
	...
	second := &array.PowerStoreArray{
		...
		BlockProtocol:      commonutils.NoneTransport,
		Insecure:           true,
		Client:             clientMock,
		IP:                 "192.168.0.2",
		NASCooldownTracker: array.NewNASCooldown(time.Minute, 5),
	...
	csictx.Setenv(context.Background(), commonutils.EnvReplicationPrefix, "replication.storage.dell.com")
	csictx.Setenv(context.Background(), commonutils.EnvNfsAcls, "A::OWNER@:RWX")
...
var _ = ginkgo.Describe("CSIControllerService", func() {
	...
	ginkgo.Describe("calling CreateVolume()", func() {
		ginkgo.When("creating block volume", func() {
			ginkgo.It("should successfully create block volume", func() {
				...
				req.Parameters[commonutils.KeyArrayID] = firstValidID
				...
				gomega.Expect(res).To(gomega.Equal(&csi.CreateVolumeResponse{
					Volume: &csi.Volume{
						...
						VolumeContext: map[string]string{
							commonutils.KeyArrayID:           firstValidID,
							commonutils.KeyArrayVolumeName:   "my-vol",
							commonutils.KeyProtocol:          "scsi",
							commonutils.KeyVolumeDescription: req.Name + "-" + validNamespaceName,
							KeyCSIPVCName:                    req.Name,
							KeyCSIPVCNamespace:               validNamespaceName,
							commonutils.KeyServiceTag:        validServiceTag,
						},
					},
				}))
			})
		...
		ginkgo.It("should successfully create block volume and vol attributes should be set", func() {
			...
			req.Parameters[KeyCSIPVCName] = req.Name
			req.Parameters[KeyCSIPVCNamespace] = validNamespaceName
			req.Parameters[commonutils.KeyVolumeDescription] = "Vol-description"
			req.Parameters[commonutils.KeyAppType] = "Other"
			req.Parameters[commonutils.KeyAppTypeOther] = "Android"
			req.Parameters[commonutils.KeyApplianceID] = "12345"
			req.Parameters[commonutils.KeyProtectionPolicyID] = "xyz"
			req.Parameters[commonutils.KeyPerformancePolicyID] = "abc"
			res, err := ctrlSvc.CreateVolume(context.Background(), req)
			...
			gomega.Expect(res).To(gomega.Equal(&csi.CreateVolumeResponse{
				Volume: &csi.Volume{
					...
					VolumeContext: map[string]string{
						...
						commonutils.KeyArrayVolumeName:     "my-vol",
						commonutils.KeyProtocol:            "scsi",
						commonutils.KeyVolumeDescription:   "Vol-description",
						commonutils.KeyAppType:             "Other",
						commonutils.KeyAppTypeOther:        "Android",
						commonutils.KeyApplianceID:         "12345",
						commonutils.KeyProtectionPolicyID:  "xyz",
						commonutils.KeyPerformancePolicyID: "abc",
						KeyCSIPVCName:                      req.Name,
						KeyCSIPVCNamespace:                 validNamespaceName,
						commonutils.KeyServiceTag:          validServiceTag,
					},
	...
	ginkgo.When("creating nfs volume", func() {
		ginkgo.It("should successfully create nfs volume", func() {
			...
			req.Parameters[commonutils.KeyArrayID] = secondValidID
			...
			gomega.Expect(res).To(gomega.Equal(&csi.CreateVolumeResponse{
				Volume: &csi.Volume{
					...
					VolumeContext: map[string]string{
						commonutils.KeyArrayVolumeName:   "my-vol",
						commonutils.KeyProtocol:          "nfs",
						commonutils.KeyArrayID:           secondValidID,
						commonutils.KeyNfsACL:            "A::OWNER@:RWX",
						commonutils.KeyNasName:           validNasName,
						commonutils.KeyVolumeDescription: req.Name + "-" + validNamespaceName,
						commonutils.KeyServiceTag:        validServiceTag,
						KeyCSIPVCName:                    req.Name,
						KeyCSIPVCNamespace:               validNamespaceName,
					},
					AccessibleTopology: []*csi.Topology{{Segments: map[string]string{commonutils.Name + "/" + ctrlSvc.Arrays()[secondValidID].GetIP() + "-nfs": "true"}}},
				},
			}))
		})
		...
		ginkgo.It("should successfully create nfs volume & all vol attribute should get set", func() {
			...
			req.Parameters[KeyCSIPVCNamespace] = validNamespaceName
			req.Parameters[commonutils.KeyVolumeDescription] = "Vol-description"
			req.Parameters[commonutils.KeyConfigType] = "ConfigType_A"
			req.Parameters[commonutils.KeyAccessPolicy] = "AccessPolicy_A"
			req.Parameters[commonutils.KeyLockingPolicy] = "KeyLockingPolicy_A"
			req.Parameters[commonutils.KeyFolderRenamePolicy] = "KeyFolderRenamePolicy"
			req.Parameters[commonutils.KeyIsAsyncMtimeEnabled] = "true"
			req.Parameters[commonutils.KeyProtectionPolicyID] = "KeyProtectionPolicyID"
			req.Parameters[commonutils.KeyFileEventsPublishingMode] = "KeyFileEventsPublishingMode"
			req.Parameters[commonutils.KeyHostIoSize] = "VMware_16K"
			req.Parameters[commonutils.KeyFlrCreateMode] = "KeyFlrCreateMode"
			req.Parameters[commonutils.KeyFlrDefaultRetention] = "KeyFlrDefaultRetention"
			req.Parameters[commonutils.KeyFlrMinRetention] = "KeyFlrMinRetention"
			req.Parameters[commonutils.KeyFlrMaxRetention] = "KeyFlrMaxRetention"
			...
			gomega.Expect(res).To(gomega.Equal(&csi.CreateVolumeResponse{
				Volume: &csi.Volume{
					...
					VolumeContext: map[string]string{
						...
						commonutils.KeyNasName:                  validNasName,
						commonutils.KeyVolumeDescription:        "Vol-description",
						commonutils.KeyConfigType:               "ConfigType_A",
						commonutils.KeyAccessPolicy:             "AccessPolicy_A",
						commonutils.KeyLockingPolicy:            "KeyLockingPolicy_A",
						commonutils.KeyFolderRenamePolicy:       "KeyFolderRenamePolicy",
						commonutils.KeyIsAsyncMtimeEnabled:      "true",
						commonutils.KeyProtectionPolicyID:       "KeyProtectionPolicyID",
						commonutils.KeyFileEventsPublishingMode: "KeyFileEventsPublishingMode",
						commonutils.KeyHostIoSize:               "VMware_16K",
						commonutils.KeyFlrCreateMode:            "KeyFlrCreateMode",
						commonutils.KeyFlrDefaultRetention:      "KeyFlrDefaultRetention",
						commonutils.KeyFlrMinRetention:          "KeyFlrMinRetention",
						commonutils.KeyFlrMaxRetention:          "KeyFlrMaxRetention",
						commonutils.KeyServiceTag:               validServiceTag,
						KeyCSIPVCName:                           req.Name,
						KeyCSIPVCNamespace:                      validNamespaceName,
					},
		...
		ginkgo.When("creating nfs volume with NFS acls in array config and storage class", func() {
			ginkgo.It("should successfully create nfs volume with storage class NFS acls in volume response", func() {
				...
				req := getTypicalCreateVolumeNFSRequest("my-vol", validVolSize)
				req.Parameters[commonutils.KeyNfsACL] = "0777"
				req.Parameters[commonutils.KeyArrayID] = secondValidID
				req.Parameters[KeyCSIPVCName] = req.Name
				req.Parameters[KeyCSIPVCNamespace] = validNamespaceName
				...
				gomega.Expect(res).To(gomega.Equal(&csi.CreateVolumeResponse{
					Volume: &csi.Volume{
						...
						VolumeContext: map[string]string{
							...
							commonutils.KeyProtocol:          "nfs",
							commonutils.KeyArrayID:           secondValidID,
							commonutils.KeyNfsACL:            "A::GROUP@:RWX",
							commonutils.KeyNasName:           validNasName,
							commonutils.KeyVolumeDescription: req.Name + "-" + validNamespaceName,
							commonutils.KeyServiceTag:        validServiceTag,
							KeyCSIPVCName:                    req.Name,
		...
		ginkgo.When("creating nfs volume with NFS acls in not in secrets & default", func() {
			ginkgo.It("should successfully create nfs volume with empty NFS acls in volume response", func() {
				...
				csictx.Setenv(context.Background(), commonutils.EnvNfsAcls, "")
		...
		ginkgo.When("creating nfs volume without nfs topology in AccessibilityRequirements", func() {
			ginkgo.It("should fail", func() {
				...
				iscsiTopology := &csi.Topology{Segments: map[string]string{commonutils.Name + "/" + ctrlSvc.Arrays()[secondValidID].GetIP() + "-iscsi": "true"}}
		...
		ginkgo.When("creating nfs volume with more than one topology in AccessibilityRequirements", func() {
			ginkgo.It("should return only nfs topology", func() {
				...
				iscsiTopology := &csi.Topology{Segments: map[string]string{commonutils.Name + "/" + ctrlSvc.Arrays()[secondValidID].GetIP() + "-iscis": "true"}}
				...
				gomega.Expect(res).To(gomega.Equal(&csi.CreateVolumeResponse{
					Volume: &csi.Volume{
						...
						VolumeContext: map[string]string{
							...
							commonutils.KeyProtocol:          "scsi",
							commonutils.KeyArrayID:           firstValidID,
							commonutils.KeyVolumeDescription: "-",
							commonutils.KeyServiceTag:        validServiceTag,
						},
					},
				}))
		...
		ginkgo.When("there array IP passed to storage class is not config", func() {
			ginkgo.It("should fail", func() {
				req := getTypicalCreateVolumeRequest("my-vol", validVolSize)
				req.Parameters[commonutils.KeyArrayID] = "127.0.0.1"
				res, err := ctrlSvc.CreateVolume(context.Background(), req)
	...
	ginkgo.When("multi-nas is enabled for NFS", func() {
		...
		ginkgo.It("should successfully create nfs volume with least used NAS if multiple NAS exist in storage class", func() {
			...
			req.Parameters[KeyCSIPVCName] = req.Name
			req.Parameters[KeyCSIPVCNamespace] = validNamespaceName
			req.Parameters[commonutils.KeyNasName] = "nasA, nasB, nasC, nasX"
		...
		ginkgo.It("should successfully create nfs volume if only one NAS exist in storage class", func() {
			...
			req.Parameters[KeyCSIPVCName] = req.Name
			req.Parameters[KeyCSIPVCNamespace] = validNamespaceName
			req.Parameters[commonutils.KeyNasName] = "nasA"
		...
		ginkgo.It("should enter a cooldown if the failure threshold (5) is reached & fallback to next best nas server", func() {
			...
			gomega.Expect(res.GetVolume().GetVolumeContext()[commonutils.KeyNasName]).To(gomega.Equal("nasB"))
		})
		...
		ginkgo.It("should be eligible for NAS selection in FS creation, after cooldown period has ended", func() {
			...
			res, err = ctrlSvc.CreateVolume(context.Background(), req)
			gomega.Expect(err).To(gomega.BeNil())
			gomega.Expect(res.GetVolume().GetVolumeContext()[commonutils.KeyNasName]).To(gomega.Equal("nasA"))
		})
	})
	...
	ginkgo.Describe("calling ControllerPublishVolume()", func() {
		...
		ginkgo.When("parameters are correct", func() {
			...
			ginkgo.It("should succeed [NFS] with externalAccess", func() {
				...
				err := csictx.Setenv(context.Background(), commonutils.EnvExternalAccess, "10.0.0.0/24")
				gomega.Expect(err).To(gomega.BeNil())
				_ = ctrlSvc.Init()
				...
				gomega.Expect(res).To(gomega.Equal(&csi.ControllerPublishVolumeResponse{
					PublishContext: map[string]string{
						commonutils.KeyNasName:       validNasName,
						commonutils.KeyNfsExportPath: secondValidID + ":/",
						commonutils.KeyExportID:      nfsID,
						commonutils.KeyAllowRoot:     "",
						commonutils.KeyHostIP:        "127.0.0.1",
						commonutils.KeyNfsACL:        "",
						commonutils.KeyNatIP:         "10.0.0.0/255.255.255.0",
					},
				}))
				// Removing externalAccess environment variable after our tests are completed
				err = csictx.Setenv(context.Background(), commonutils.EnvExternalAccess, "")
				gomega.Expect(err).To(gomega.BeNil())
				_ = ctrlSvc.Init()
			})
		})
		...
		ginkgo.When("using nfs nat feature", func() {
			ginkgo.It("should succeed", func() {
				...
				_ = csictx.Setenv(context.Background(), commonutils.EnvExternalAccess, externalAccess)
				_ = ctrlSvc.Init()
	...
	ginkgo.Describe("calling ControllerUnpublishVolume()", func() {
		...
		ginkgo.It("should succeed [NFS] by removing external access from the HostAccessList", func() {
			// setting externalAccess environment variable
			err := csictx.Setenv(context.Background(), commonutils.EnvExternalAccess, "10.0.0.0/16")
			gomega.Expect(err).To(gomega.BeNil())
			_ = ctrlSvc.Init()
			clientMock.On("GetFsSnapshotsByVolumeID", mock.Anything, validBaseVolID).Return([]gopowerstore.FileSystem{}, nil)
	...
	ginkgo.Describe("calling ControllerGetCapabilities()", func() {
		ginkgo.When("plugin functions correctly with health monitor capabilities", func() {
			ginkgo.It("should return supported capabilities", func() {
				csictx.Setenv(context.Background(), commonutils.EnvIsHealthMonitorEnabled, "true")
				ctrlSvc.Init()
		...
		ginkgo.When("plugin functions correctly without health monitor capabilities", func() {
			ginkgo.It("should return supported capabilities", func() {
				csictx.Setenv(context.Background(), commonutils.EnvIsHealthMonitorEnabled, "false")
				ctrlSvc.Init()
				res, err := ctrlSvc.ControllerGetCapabilities(context.Background(), &csi.ControllerGetCapabilitiesRequest{})
				gomega.Expect(err).To(gomega.BeNil())
				gomega.Expect(res).To(gomega.Equal(&csi.ControllerGetCapabilitiesResponse{
...
func getTypicalCreateVolumeNFSRequest(name string, size int64) *csi.CreateVolumeRequest {
	...
	nfsTopology := &csi.Topology{Segments: map[string]string{commonutils.Name + "/" + ctrlSvc.Arrays()[secondValidID].GetIP() + "-nfs": "true"}}
