...
var _ = ginkgo.Describe("CSIControllerService", func() {
	...
	ginkgo.Describe("calling CreateVolume()", func() {
				...
				gomega.Expect(res).To(gomega.Equal(&csi.CreateVolumeResponse{
					Volume: &csi.Volume{
						...
						VolumeContext: map[string]string{
							identifiers.KeyArrayID:           firstValidID,
							identifiers.KeyArrayVolumeName:   "my-vol",
							identifiers.KeyProtocol:          "scsi",
							identifiers.KeyVolumeDescription: req.Name + "-" + validNamespaceName,
							KeyCSIPVCName:                    req.Name,
							KeyCSIPVCNamespace:               validNamespaceName,
							identifiers.KeyServiceTag:        validServiceTag,
						},
					},
				}))
			})
		...
		ginkgo.It("should successfully create block volume and vol attributes should be set", func() {
			...
			req.Parameters[KeyCSIPVCName] = req.Name
			req.Parameters[KeyCSIPVCNamespace] = validNamespaceName
			req.Parameters[identifiers.KeyVolumeDescription] = "Vol-description"
			req.Parameters[identifiers.KeyAppType] = "Other"
			req.Parameters[identifiers.KeyAppTypeOther] = "Android"
			req.Parameters[identifiers.KeyApplianceID] = "12345"
			req.Parameters[identifiers.KeyProtectionPolicyID] = "xyz"
			req.Parameters[identifiers.KeyPerformancePolicyID] = "abc"
			res, err := ctrlSvc.CreateVolume(context.Background(), req)
			...
			gomega.Expect(res).To(gomega.Equal(&csi.CreateVolumeResponse{
				Volume: &csi.Volume{
					...
					VolumeContext: map[string]string{
						...
						identifiers.KeyArrayVolumeName:     "my-vol",
						identifiers.KeyProtocol:            "scsi",
						identifiers.KeyVolumeDescription:   "Vol-description",
						identifiers.KeyAppType:             "Other",
						identifiers.KeyAppTypeOther:        "Android",
						identifiers.KeyApplianceID:         "12345",
						identifiers.KeyProtectionPolicyID:  "xyz",
						identifiers.KeyPerformancePolicyID: "abc",
						KeyCSIPVCName:                      req.Name,
						KeyCSIPVCNamespace:                 validNamespaceName,
						identifiers.KeyServiceTag:          validServiceTag,
					},
	...
	ginkgo.When("creating nfs volume", func() {
		ginkgo.It("should successfully create nfs volume", func() {
			...
			gomega.Expect(res).To(gomega.Equal(&csi.CreateVolumeResponse{
				Volume: &csi.Volume{
					...
					VolumeContext: map[string]string{
						identifiers.KeyArrayVolumeName:   "my-vol",
						identifiers.KeyProtocol:          "nfs",
						identifiers.KeyArrayID:           secondValidID,
		...
		ginkgo.It("should successfully create nfs volume & all vol attribute should get set", func() {
			...
			req.Parameters[KeyCSIPVCNamespace] = validNamespaceName
			req.Parameters[identifiers.KeyVolumeDescription] = "Vol-description"
			req.Parameters[identifiers.KeyConfigType] = "ConfigType_A"
			req.Parameters[identifiers.KeyAccessPolicy] = "AccessPolicy_A"
			req.Parameters[identifiers.KeyLockingPolicy] = "KeyLockingPolicy_A"
			...
			req.Parameters[identifiers.KeyFlrDefaultRetention] = "KeyFlrDefaultRetention"
			req.Parameters[identifiers.KeyFlrMinRetention] = "KeyFlrMinRetention"
			req.Parameters[identifiers.KeyFlrMaxRetention] = "KeyFlrMaxRetention"
			...
			gomega.Expect(res).To(gomega.Equal(&csi.CreateVolumeResponse{
				Volume: &csi.Volume{
					...
					VolumeContext: map[string]string{
						...
						identifiers.KeyNasName:                  validNasName,
						identifiers.KeyVolumeDescription:        "Vol-description",
						identifiers.KeyConfigType:               "ConfigType_A",
						identifiers.KeyAccessPolicy:             "AccessPolicy_A",
						identifiers.KeyLockingPolicy:            "KeyLockingPolicy_A",
						...
						identifiers.KeyFlrDefaultRetention:      "KeyFlrDefaultRetention",
						identifiers.KeyFlrMinRetention:          "KeyFlrMinRetention",
						identifiers.KeyFlrMaxRetention:          "KeyFlrMaxRetention",
						identifiers.KeyServiceTag:               validServiceTag,
						KeyCSIPVCName:                           req.Name,
						KeyCSIPVCNamespace:                      validNamespaceName,
					},
		...
		ginkgo.When("creating nfs volume with NFS acls in array config and storage class", func() {
			ginkgo.It("should successfully create nfs volume with storage class NFS acls in volume response", func() {
				...
				req := getTypicalCreateVolumeNFSRequest("my-vol", validVolSize)
				req.Parameters[identifiers.KeyNfsACL] = "0777"
				req.Parameters[identifiers.KeyArrayID] = secondValidID
				req.Parameters[KeyCSIPVCName] = req.Name
				req.Parameters[KeyCSIPVCNamespace] = validNamespaceName
				...
				gomega.Expect(res).To(gomega.Equal(&csi.CreateVolumeResponse{
					Volume: &csi.Volume{
						...
						VolumeContext: map[string]string{
							...
							identifiers.KeyProtocol:          "nfs",
							identifiers.KeyArrayID:           secondValidID,
							identifiers.KeyNfsACL:            "A::GROUP@:RWX",
							identifiers.KeyNasName:           validNasName,
							identifiers.KeyVolumeDescription: req.Name + "-" + validNamespaceName,
							identifiers.KeyServiceTag:        validServiceTag,
							KeyCSIPVCName:                    req.Name,
		...
		ginkgo.When("creating nfs volume without nfs topology in AccessibilityRequirements", func() {
			ginkgo.It("should fail", func() {
				...
				iscsiTopology := &csi.Topology{Segments: map[string]string{identifiers.Name + "/" + ctrlSvc.Arrays()[secondValidID].GetIP() + "-iscsi": "true"}}
		...
		ginkgo.When("creating nfs volume with more than one topology in AccessibilityRequirements", func() {
			ginkgo.It("should return only nfs topology", func() {
				...
				iscsiTopology := &csi.Topology{Segments: map[string]string{identifiers.Name + "/" + ctrlSvc.Arrays()[secondValidID].GetIP() + "-iscis": "true"}}
				...
				gomega.Expect(res).To(gomega.Equal(&csi.CreateVolumeResponse{
					Volume: &csi.Volume{
						...
						VolumeContext: map[string]string{
							...
							identifiers.KeyProtocol:          "scsi",
							identifiers.KeyArrayID:           firstValidID,
							identifiers.KeyVolumeDescription: "-",
							identifiers.KeyServiceTag:        validServiceTag,
						},
					},
				}))
		...
		ginkgo.When("there array IP passed to storage class is not config", func() {
			ginkgo.It("should fail", func() {
				req := getTypicalCreateVolumeRequest("my-vol", validVolSize)
				req.Parameters[identifiers.KeyArrayID] = "127.0.0.1"
				res, err := ctrlSvc.CreateVolume(context.Background(), req)
	...
	ginkgo.When("multi-nas is enabled for NFS", func() {
		...
		ginkgo.It("should successfully create nfs volume if only one NAS exist in storage class", func() {
			...
			req.Parameters[KeyCSIPVCName] = req.Name
			req.Parameters[KeyCSIPVCNamespace] = validNamespaceName
			req.Parameters[identifiers.KeyNasName] = "nasA"
		...
		ginkgo.It("should enter a cooldown if the failure threshold (5) is reached & fallback to next best nas server", func() {
			...
			gomega.Expect(res.GetVolume().GetVolumeContext()[identifiers.KeyNasName]).To(gomega.Equal("nasB"))
		})
		...
		ginkgo.It("should be eligible for NAS selection in FS creation, after cooldown period has ended", func() {
			...
			res, err = ctrlSvc.CreateVolume(context.Background(), req)
			gomega.Expect(err).To(gomega.BeNil())
			gomega.Expect(res.GetVolume().GetVolumeContext()[identifiers.KeyNasName]).To(gomega.Equal("nasA"))
		})
	})
	...
	ginkgo.Describe("calling ControllerPublishVolume()", func() {
		...
		ginkgo.When("using nfs nat feature", func() {
			ginkgo.It("should succeed", func() {
				...
				_ = csictx.Setenv(context.Background(), identifiers.EnvExternalAccess, externalAccess)
				_ = ctrlSvc.Init()
...
func getTypicalCreateVolumeNFSRequest(name string, size int64) *csi.CreateVolumeRequest {
	...
	nfsTopology := &csi.Topology{Segments: map[string]string{identifiers.Name + "/" + ctrlSvc.Arrays()[secondValidID].GetIP() + "-nfs": "true"}}
