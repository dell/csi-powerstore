...
type PowerStoreArray struct {
	...
	BlockProtocol commonutils.TransportType `yaml:"blockProtocol"`
...
func GetPowerStoreArrays(fs fs.Interface, filePath string) (map[string]*PowerStoreArray, map[string]string, *PowerStoreArray, error) {
	...
	for _, array := range cfg.Arrays {
		...
		if throttlingRateLimit, ok := csictx.LookupEnv(context.Background(), commonutils.EnvThrottlingRateLimit); ok {
		...
		c.SetCustomHTTPHeaders(http.Header{
			"Application-Type": {fmt.Sprintf("%s/%s", commonutils.VerboseName, core.SemVer)},
		})

		c.SetLogger(&commonutils.CustomLogger{})
		array.Client = c

		if array.BlockProtocol == "" {
			array.BlockProtocol = commonutils.AutoDetectTransport
		}
		array.BlockProtocol = commonutils.TransportType(strings.ToUpper(string(array.BlockProtocol)))
		var ip string
		ips := commonutils.GetIPListFromString(array.Endpoint)
		if ips == nil {
		...
		if threshold, ok := csictx.LookupEnv(context.Background(), commonutils.EnvMultiNASFailureThreshold); ok {
		...
		if cp, ok := csictx.LookupEnv(context.Background(), commonutils.EnvMultiNASCooldownPeriod); ok {
...
func ParseVolumeID(ctx context.Context, volumeHandleRaw string,
	...
	if len(localVolumeHandle) == 1 {
		...
		if ips := commonutils.GetIPListFromString(localVolumeHandle[1]); ips != nil {
