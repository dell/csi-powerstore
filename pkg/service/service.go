...
func (s *service) BeforeServe(ctx context.Context, _ *gocsi.StoragePlugin, _ net.Listener) error {
	...
	nodeName := os.Getenv(powerstorecommon.EnvKubeNodeName)
	if nodeName == "" {
	...
	if s.mode == "node" {
		nodeRoot := os.Getenv(powerstorecommon.EnvNodeChrootPath)
		if nodeRoot == "" {
	...
	// The block is commented out for performance issue caused by sharednfs even when not enabled.
