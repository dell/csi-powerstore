...
func New(controllerSvc controller.Interface, identitySvc *identity.Service, nodeSvc node.Interface, interList []grpc.UnaryServerInterceptor) *gocsi.StoragePlugin {
	...
	nfssvc := nfs.New(identifiers.Name)
	service.PutNfsService(nfssvc)
	nfs.PutVcsiService(svc)
	nfs.DriverName = identifiers.Name

	driverNamespace := os.Getenv(identifiers.EnvDriverNamespace)
	if driverNamespace != "" {
		Log.Infof("Reading driver namespace from env variable %s", identifiers.EnvDriverNamespace)
	...
	nfs.NfsExportDirectory = os.Getenv(identifiers.EnvNFSExportDirectory)
